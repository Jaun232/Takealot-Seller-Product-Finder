...
const SellerSearchForm: React.FC<SellerSearchFormProps> = ({ onSearch, isLoading }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (sellerId.trim() && !isLoading) {
      onSearch(sellerId.trim());
    }
  };
  ...
  return (
    <form
      ...
      className="w-full flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 p-2 bg-gray-800 rounded-lg shadow-md"
    >
      <input
        type="text"
        value={sellerId}
